...
def show_captured_frame(captured_frame):  # Display the captured frame
    cv2.imshow("Preprocessed Frame", captured_frame)  # Show the image
    cv2.waitKey(50)  # Wait briefly to ensure the image displays

def close_camera(camera):  # Close the camera
    camera.release()  # Release the camera resource

def move_bin1(camera, frame, label):  # Move object to bin 1 (right)
    show_captured_frame(frame)  # Display the captured image
    print("MOVING: ", label)  # Print the detected object
    fetch_object()  # Pick up the object
    right45()  # Move to right 45 degrees
    release_object()  # Release the object
    center()  # Return to center
    close_camera(camera)  # Close camera to clear the last frame
    camera = initialize_camera()  # Reopen the camera
    return camera  # Return the new camera object

def move_bin2(camera, frame, label):  # Move object to bin 2 (left)
    show_captured_frame(frame)  # Display the captured image
    print("MOVING: ", label)  # Print the detected object
    fetch_object()  # Pick up the object
    left45()  # Move to left 45 degrees
    release_object()  # Release the object
    center()  # Return to center
    close_camera(camera)  # Close camera to clear the last frame
    camera = initialize_camera()  # Reopen the camera
...
def main():  # Main function
    camera = initialize_camera()  # Initialize the camera
